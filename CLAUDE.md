...
- All list endpoints take a `Pageable` param (`@PageableDefault` for sort/direction) instead of a hand-parsed `page` string or a hardcoded size — `spring.data.web.pageable.*` in `application.yml` sets the shared default size (10) and hard cap (50). When testing a controller method with a `Pageable` param via `MockMvcBuilders.standaloneSetup(...)`, you must add `.setCustomArgumentResolvers(new PageableHandlerMethodArgumentResolver())` — the full app context resolves it automatically via Spring Boot's data-web autoconfiguration, but a standalone MockMvc build doesn't wire that up, and the request 500s ("No primary or single unique constructor found for interface ... Pageable") without it.
...
- Tests live under `src/test/java/com/example/social_be/`, organized by the package they cover (`controller/`, `service/`, `config/`, `util/`, `exception/`). Most are plain Mockito/MockMvc-standalone unit tests that don't need Mongo or a Spring context at all. `SocialBeApplicationTests` and `SecurityIntegrationTest` (root package) extend `AbstractIntegrationTest`, which boots the full `@SpringBootTest` context against a real, ephemeral Mongo via Testcontainers (`@ServiceConnection` wires the connection automatically) — no manually-provided `application.properties` needed; `src/test/resources/application.yml` supplies dummy-but-valid values for the rest of `SocialAppProperties`/mail. `SecurityIntegrationTest` drives the *real* filter chain (`JwtRequestFilter` + `SecurityConfiguration`, real signed JWT cookies via `JwtTokenUtil`) rather than a standalone controller instance — the one place a broken `requestMatchers()` rule would actually get caught. **Requires a Docker daemon reachable from the JVM running the tests** (works on GitHub Actions and a normal native Docker host); a Docker-Desktop-on-WSL2 setup with the daemon socket bind-mounted into a *nested* container can fail Testcontainers' own environment detection with `Could not find a valid Docker environment` even though the socket itself is reachable (confirmed via raw `curl --unix-socket` during REF-3) — that's an environment limitation, not a code problem, and doesn't affect a normal CI runner or dev machine.

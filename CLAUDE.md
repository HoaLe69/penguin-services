...
- Don't add `@Async` or `@Transactional` to controller methods. Mongo here is a standalone instance, not a replica set, so `@Transactional` has no real effect anywhere in this codebase yet (REF-20 tracks fixing that); `@Async` on a method returning `ResponseEntity` doesn't do what it looks like it does either. Both were removed from every controller for exactly this reason.
